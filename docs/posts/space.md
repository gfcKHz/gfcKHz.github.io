...
The aim is to make the rotational lattice auditable: a discrete engine for latent space where any combination of stems respects both the Heilbronn area budget and the tonnetz symmetries. Each musician’s chart supplies a unique angular signature on that lattice; mapping those angles to pitch classes fixes which tonal paths their stems may traverse. When we feed the same capture rig's micro-fluctuations (drift, jitter, PLL relock cadence) into the lattice as auxiliary coordinates, the stem splitter behaves like an audio side-channel: every hallucinated stem carries the user's geometric and hardware [fingerprint](entropy.md). 
